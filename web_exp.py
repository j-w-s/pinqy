...
def parse_sitemap(soup: BeautifulSoup) -> List[str]:
    return pinqy(soup.find_all('loc')).select(lambda tag: tag.get_text(strip=True)).where(lambda url: url).to.list()
...
def element_to_functional_dict(element) -> Optional[Dict[str, Any]]:
    if not hasattr(element, 'name'):
        ...
        return {'type': 'text', 'content': text_content, 'length': len(text_content),
                'has_numbers': bool(re.search(r'\d', text_content)),
                'has_currency': bool(re.search(r'[\$\€\£\¥]', text_content)), 'word_count': len(text_content.split())}
    ...
    return {'type': 'element', 'tag': element.name, 'attrs': attrs, 'classes': attrs.get('class', []),
            'id': attrs.get('id'), 'text': text, 'text_length': len(text), 'children': children,
            'children_count': len(children), 'has_text': bool(text), 'has_children': bool(children),
...
def create_structural_signature(elem: Dict[str, Any]) -> str:
    ...
    has_text, has_links, children_count = ('T' if elem.get('has_text') else 'N'), (
        'L' if elem.get('has_links') else 'N'), min(elem.get('children_count', 0), 10)
...
def analyze_page_semantics(tree: Dict[str, Any]) -> Dict[str, Any]:
    ...
    product_keywords, article_keywords, contact_keywords = {'price', 'cart', 'add', 'sku', 'sale', '$', '€', '£'}, {
        'author', 'published', 'updated', 'by', 'copyright', 'comments'}, {'contact', 'address', 'phone', 'email',
                                                                           'hours', 'location'}
    words = pinqy(full_text.split())
    return {'product_score': words.where(lambda w: w in product_keywords).set.distinct().to.count(),
...
def discover_data_fields_functional(elements: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """discover extractable data fields using functional composition with improved logic."""
    ...
    def extract_text_paths(elem: Dict[str, Any], path: str = "") -> List[Dict[str, Any]]:
        """recursively find text paths, preferring the deepest node with unique text."""
        paths = []
        if elem.get('type') != 'element': return paths

        tag = elem.get('tag', '')
        classes = '.'.join(elem.get('classes', [])[:2])
        current_path = f"{path}.{tag}" + (f".{classes}" if classes else "")

        # heuristic: is this node just a container for a single child with the same text?
        is_container_node = pinqy(elem.get('children', [])) \
            .where(lambda c: c.get('type') == 'element' and c.get('text') == elem.get('text')) \
            .to.any()

        # if it has text and it's not just a simple container, it's a candidate field.
        if elem.get('text') and not is_container_node:
            paths.append({'path': current_path, 'content': elem['text']})

        # always recurse to find deeper, more specific fields.
        paths.extend(
            pinqy(elem.get('children', [])).select_many(lambda c: extract_text_paths(c, current_path)).to.list())
        return paths

    all_paths = sample_elements.select_many(lambda e: extract_text_paths(e)).to.list()
    if not all_paths: return []

    path_groups = pinqy(all_paths).group.group_by(lambda p: p['path'])
    return (pinqy(path_groups.values())
            .where(lambda group: len(group) >= num_samples * 0.5)
            .select(lambda group: {
...
def process_url_functionally(url: str, session: requests.Session, robots_checker: Callable[[str], bool],
    ...
    if content_type == 'xml':
        ...
        absolute_links = pinqy(sitemap_links).select(lambda link: urljoin(url, link)).where(
            lambda abs_url: urlparse(abs_url).netloc == base_domain).set.distinct().to.list()
        return {'url': url, 'title': 'sitemap', 'classification': {'type': 'sitemap', 'confidence': 1.0},
                'extraction': {'extracted_data': [], 'links': sitemap_links}, 'child_links': absolute_links,
        ...
        if not dominant_pattern:
            ...
            ).select(lambda link_element: link_element.get('href')).set.distinct().to.list()
        absolute_links = pinqy(links).select(lambda link: urljoin(url, link)).where(
            lambda abs_url: urlparse(abs_url).netloc == base_domain).set.distinct().to.list()
        return {'url': url, 'title': tree.get('title', ''), 'classification': classification,
                'extraction': {'extracted_data': extracted_data, 'links': links}, 'child_links': absolute_links,
                'processed_at': datetime.now().isoformat()}
    return None
...
def explore_domain_functionally(start_urls: List[str], config: ExplorationConfig) -> Dict[str, Any]:
    ...
    while not url_queue.empty() and pages_processed < config.max_pages_per_domain:
        ...
        for result in batch_results:
            ...
            priority_score = {'sitemap': 0, 'list': 1, 'directory': 1, 'content_list': 2, 'structured': 3,
                              'detail': 4}.get(page_type, 5)
    ...
    return {'domain': domain, 'total_pages_processed': pages_processed, 'total_urls_visited': len(visited_urls),
    ...
    type_summary = pinqy(page_type_analysis.items()).to.dict(
        ...
        value_selector=lambda item: (
            ...
            {'page_count': pages.to.count(),
             ...
             'total_data_items': all_data.to.count(), 'avg_items_per_page': pages.stats.average(
             ...
             'discovered_fields': list(all_data.to.first().keys()) if all_data.to.any() and all_data.to.first() else []}
        )())
    return {'summary': {'total_pages': results.to.count(), 'total_data_items': results.stats.sum(
        lambda r: len(r.get('extraction', {}).get('extracted_data', []))),
                        ...
                        'pages_with_data': results.where(
                            lambda r: r.get('extraction', {}).get('extracted_data')).to.count()},
    ...
    if config.output_format == 'json':
        ...
        all_data = pinqy(exploration_results.get('results', [])).select_many(
            lambda r: [{'source_url': r['url'], 'page_type': r['classification']['type'], **item} for item in
                       r.get('extraction', {}).get('extracted_data', [])]).to.list()
        ...
        if config.output_format == 'csv':
            ...
            with sqlite3.connect(filename) as conn:
                ...
                for result in exploration_results['results']:
                    ...
                    if page_id:
                        for item in result.get('extraction', {}).get('extracted_data', []): conn.execute(
...
class FunctionalQueryBuilder:
    def __init__(self, exploration_results: Dict[str, Any]):
        self.results = pinqy(
            exploration_results.get('results', [])); self.current_query = self.results; self.query_history = []

    def filter_by_page_type(self, page_type: str):
        self.current_query = self.current_query.where(
            lambda r: page_type in r['classification']['type']); self.query_history.append(
            f"filter_by_page_type('{page_type}')"); return self

    def select_data_items(self):
        self.current_query = self.current_query.select_many(
            lambda r: [{'source_url': r['url'], 'page_type': r['classification']['type'], **item} for item in
                       r.get('extraction', {}).get('extracted_data', [])]); self.query_history.append(
            "select_data_items()"); return self
